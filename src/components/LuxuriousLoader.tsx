...
export default function LuxuriousLoader({ fullScreen = false, size = 'md', text }: LuxuriousLoaderProps) {
  const containerClasses = fullScreen
    ? "min-h-screen w-full bg-gray-50 dark:bg-[#090D16] flex flex-col items-center justify-center p-4 relative overflow-hidden"
  ...
  return (
    <div className={containerClasses}>
      ...
      {fullScreen && (
        <>
          <div className="absolute -top-40 -left-40 w-96 h-96 rounded-full bg-sky-500/5 dark:bg-cyan-400/5 blur-3xl pointer-events-none" />
        ...
        >
          <span className={`${selectedSize.font} font-black text-gray-800 dark:text-gray-100 flex items-center gap-2`}>
            ...
            <span className="flex gap-0.5">
              <motion.span animate={{ opacity: [0.2, 1, 0.2] }} transition={{ repeat: Infinity, duration: 1.2, delay: 0 }} className="w-1 h-1 rounded-full bg-sky-600 dark:bg-cyan-500" />
              <motion.span animate={{ opacity: [0.2, 1, 0.2] }} transition={{ repeat: Infinity, duration: 1.2, delay: 0.25 }} className="w-1 h-1 rounded-full bg-sky-600 dark:bg-cyan-500" />
              <motion.span animate={{ opacity: [0.2, 1, 0.2] }} transition={{ repeat: Infinity, duration: 1.2, delay: 0.5 }} className="w-1 h-1 rounded-full bg-sky-600 dark:bg-cyan-500" />
            </span>
          </span>
