...
export default function StudentQudurat({ userData, setUserData, initialSelectedReviewId }: StudentQuduratProps) {
  ...
  const handlePurchaseClick = (review: QuduratReview) => {
    if (!userData) {
      toast.error('يرجى تسجيل حساب أولاً للمتابعة والاشتراك في المراجعة.');
      navigate('/special-register?type=qudurat');
      return;
    }
  ...
  const handleWalletPayment = async () => {
    ...
    try {
      ...
      await updateDoc(doc(db, 'qudurat_reviews', selectedReview.id), {
      ...
      await addDoc(collection(db, 'wallet_transactions'), {
        ...
        description: `شراء مراجعة القدرات: ${selectedReview.title}`,
  ...
  return (
      ...
      <AnimatePresence mode="wait">
        {!selectedReview ? (
          ...
          >
            ...
            {/* List Section */}
            {reviews.length === 0 ? (
              <div className="text-center py-16 bg-white dark:bg-[#111827] rounded-3xl border border-gray-100 dark:border-slate-800 shadow-sm max-w-md mx-auto p-8 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {reviews.map((review) => {
                    ...
                    const cardStyleClass = isPdf 
                      ...
                      : 'border-gray-150 dark:border-slate-800 hover:border-purple-300 dark:hover:border-purple-800/80 shadow-gray-100/10 dark:shadow-none hover:shadow-purple-500/5';

                    return (
                      <motion.div
                        key={review.id}
                        whileHover={{ y: -5 }}
                        className={`bg-white dark:bg-[#111827] border ${cardStyleClass} rounded-3xl overflow-hidden shadow-md hover:shadow-xl transition-all flex flex-col justify-between group cursor-pointer`}
                      ...
                      >
                        ...
                        <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
                          ...
                          <div className="flex items-center justify-between text-[10px] text-gray-400 dark:text-gray-500 font-bold border-t border-b border-gray-50 dark:border-slate-800 py-2.5">
    ...
    <div className="bg-slate-950 border border-gray-150 dark:border-slate-800 rounded-3xl overflow-hidden shadow-2xl relative aspect-video">
                      {/* Video embed / player */}
    ...
    <div className="w-full h-full flex flex-col items-center justify-center bg-gray-50 dark:bg-[#0D121F] border border-gray-150 dark:border-slate-800 p-8 rounded-3xl shadow-xl min-h-[300px] mb-6">
  ...
  {/* Review Info Tab */}
                    <div className="bg-white dark:bg-[#111827] border border-gray-150 dark:border-slate-800 rounded-3xl p-6 sm:p-8 shadow-sm space-y-4">
...
الدرس النشط حالياً: {activeLesson?.title}
                      ...
                      {(review.pdfUrl || review.examId) && (
                        <div className="pt-4 border-t border-gray-100 dark:border-slate-800 flex flex-wrap gap-3">
                          {review.pdfUrl && (
                            <button
                              onClick={() => {
                  ...
                  <div className="space-y-6">
                    ...
                    {purchased && (!review.contentType || review.contentType === 'video_course') && (
                      <div className="bg-white dark:bg-[#111827] border border-gray-150 dark:border-slate-800 rounded-3xl p-6 shadow-sm space-y-4">
                    ...
                    {(!review.contentType || review.contentType === 'video_course') && (
                      <div className="bg-white dark:bg-[#111827] border border-gray-150 dark:border-slate-800 rounded-3xl overflow-hidden shadow-sm flex flex-col">
                        <div className="p-4 bg-gray-50 dark:bg-[#1C1C28] border-b border-gray-100 dark:border-slate-800">
                  ...
                  <AnimatePresence mode="wait">
                    ...
                    >
                      <div className="bg-purple-50/50 dark:bg-purple-950/10 border border-purple-100 dark:border-purple-900/30 rounded-3xl p-6 text-center space-y-4">
                        <div className="w-16 h-16 bg-white dark:bg-[#090D16] rounded-2xl flex items-center justify-center mx-auto shadow-sm border border-purple-100 dark:border-purple-900/30">
                      ...
                      <div className="space-y-4">
                        ...
                        {userData && (userData.balance || 0) < (selectedReview.discountPrice || selectedReview.price || 0) ? (
                          ...
                          <div className="p-4 rounded-2xl bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-900/30 flex gap-3">
                            ...
                            <p className="text-[11px] font-bold text-emerald-700 dark:text-amber-400 leading-relaxed">
                      ...
                      <form onSubmit={handlePaymentSubmit} className="p-6 space-y-5">
                        ...
                        <div className="bg-gray-50/50 dark:bg-[#090D16]/50 border border-gray-100 dark:border-slate-800 rounded-2xl p-4 space-y-4">
                          ...
                          <div className="space-y-3">
                            ...
                            {customPaymentMethods?.filter(m => m.isEnabled).map(method => (
                              <div key={method.id} className="flex flex-col gap-2 bg-white dark:bg-[#0D121F] border border-emerald-100 dark:border-emerald-900/30 rounded-xl p-3">
                                ...
                                <div className="flex items-start justify-between">
                                  ...
                                  <button
                                    ...
                                    className="p-2 bg-gray-50 hover:bg-emerald-50 dark:bg-[#111827] dark:hover:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400 rounded-lg transition-all active:scale-95 border border-gray-100 dark:border-slate-800 shrink-0 mr-3"
                                  >
                                    <Copy className="w-4 h-4" />
                                  </button>
                                </div>
                        ...
                        <div className="space-y-1.5">
                          ...
                          <input
                            ...
                            className="w-full bg-gray-50 dark:bg-[#090D16] border border-gray-200 dark:border-slate-800 rounded-xl px-4 py-3 outline-none focus:border-purple-600 dark:text-white font-bold text-xs font-sans"
                          />
                        </div>
                        ...
                        <div className="space-y-1.5">
                          ...
                          <input
                            ...
                            className="w-full bg-gray-50 dark:bg-[#090D16] border border-gray-200 dark:border-slate-800 rounded-xl px-4 py-3 outline-none focus:border-purple-600 dark:text-white font-bold text-xs font-sans text-right"
                            dir="ltr"
                          />
                        </div>
