    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-black text-gray-900 dark:text-white flex items-center gap-2">
          <MessageSquare className="w-6 h-6 text-sky-600 dark:text-cyan-400" />
          طلبات المراسلة (الدعم الفني)
        </h2>
        <div className="bg-white dark:bg-[#111827] px-4 py-2 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-sm flex items-center gap-2">
          <span className="text-sm font-bold text-gray-600 dark:text-gray-300">إجمالي الطلبات:</span>
          <span className="bg-sky-500/10 dark:bg-cyan-400/10 text-sky-600 dark:text-cyan-400 px-2 py-1 rounded-lg text-sm font-black">
            {requests.length}
          </span>
        </div>
      </div>
      ...
      {requests.length === 0 ? (
        <div className="text-center py-16 bg-white dark:bg-[#111827] rounded-3xl border border-gray-100 dark:border-slate-800">
        ...
        <div className="grid gap-4">
          {requests.map(request => (
            <div 
              key={request.id} 
              className={`bg-white dark:bg-[#111827] p-6 rounded-3xl border transition-all ${
                request.status === 'resolved' 
                  ? 'border-gray-100 dark:border-slate-800 opacity-75' 
                  : 'border-sky-500/30 dark:border-cyan-400/30 shadow-md'
              }`}
            >
              ...
              <div className="bg-gray-50 dark:bg-[#090D16] p-4 rounded-2xl border border-gray-100 dark:border-slate-800 text-right">
