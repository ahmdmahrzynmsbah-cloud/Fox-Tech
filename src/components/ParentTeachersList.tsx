...
const ParentTeachersList: React.FC<ParentTeachersListProps> = ({ userData, linkedStudent }) => {
  ...
  if (!linkedStudent) {
    return (
      <div className="bg-white dark:bg-[#111827] rounded-3xl p-12 border border-gray-200 dark:border-slate-800 shadow-sm text-center">
  ...
  if (teachers.length === 0) {
    return (
      <div className="bg-white dark:bg-[#111827] rounded-3xl p-12 border border-gray-200 dark:border-slate-800 shadow-sm text-center">
        <div className="w-20 h-20 bg-blue-50 dark:bg-blue-950/20 text-sky-600 dark:text-cyan-400 rounded-3xl flex items-center justify-center mx-auto mb-6">
          <BookOpen className="w-10 h-10" />
        </div>
        <h2 className="text-2xl font-black text-gray-900 dark:text-white mb-2">لا يوجد معلمون حالياً</h2>
        <p className="text-gray-500 dark:text-gray-400 font-medium mb-6">يبدو أن ابنك <span className="text-sky-600 dark:text-cyan-400 font-black">{linkedStudent.name}</span> لم يشترك في أي كورس بعد.</p>
      </div>
    );
  }
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-black text-gray-900 dark:text-white flex items-center gap-2">
            <Users className="w-7 h-7 text-sky-600 dark:text-cyan-400" /> تواصل مع معلمي {linkedStudent.name.split(' ')[0]}
          </h2>
        ...
        <div className="relative w-full md:w-72">
          ...
          <input
            ...
            className="w-full bg-white dark:bg-[#15151F] border border-gray-200 dark:border-slate-800 rounded-2xl pr-10 pl-4 py-2.5 outline-none focus:border-sky-500 dark:focus:border-cyan-500 text-sm font-bold text-gray-900 dark:text-white transition-all"
          />
        </div>
      </div>
        ...
        <AnimatePresence mode="popLayout">
          {filteredTeachers.map((teacher, index) => (
            <motion.div
              ...
              className="bg-white dark:bg-[#111827] rounded-3xl border border-gray-100 dark:border-slate-800 shadow-sm hover:shadow-md transition-all p-6 flex flex-col items-center text-center relative overflow-hidden group"
            >
              ...
              <div className="w-20 h-20 rounded-2xl bg-gray-100 dark:bg-[#090D16] flex items-center justify-center mb-4 relative z-10 border-2 border-white dark:border-slate-800 shadow-sm">
                <span className="text-3xl font-black text-sky-600 dark:text-cyan-400">
                  {teacher.name.charAt(0)}
                </span>
              </div>

              <h3 className="text-lg font-black text-gray-900 dark:text-white mb-1 relative z-10">{teacher.name}</h3>
              <p className="text-xs font-bold text-sky-600 dark:text-cyan-400 mb-4 bg-blue-50 dark:bg-blue-900/30 px-3 py-1 rounded-lg">
                {teacher.subject || 'معلم متخصص'}
              </p>

              <div className="w-full space-y-2 mb-6 relative z-10">
                <div className="flex items-center justify-between text-xs font-bold p-3 rounded-xl bg-gray-50 dark:bg-[#090D16] border border-gray-100 dark:border-slate-800">
