...
export default function TeacherTahsili({ userData }: TeacherTahsiliProps) {
  const [reviews, setReviews] = useState<TahsiliReview[]>([]);
  const [loading, setLoading] = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [editingReview, setEditingReview] = useState<TahsiliReview | null>(null);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      toast.error('خطأ في تحميل بيانات مراجعات التحصيلي');
      setLoading(false);
    });
  ...
  const handleOpenCreate = () => {
    ...
    setThumbnail('');
    setPromoImage('');
    setSubject(userData.subject || 'البرمجة');
  ...
  const handleOpenEdit = (review: TahsiliReview) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Error saving Tahsili review:', err);
  ...
  const confirmDelete = async (id: string) => {
    try {
      await deleteDoc(doc(db, 'tahsili_reviews', id));
      toast.success('تم حذف المراجعة بنجاح ✨');
    } catch (err) {
      console.error('Error deleting Tahsili review:', err);
  ...
  // Quick toggle status
  const handleToggleStatus = async (review: TahsiliReview) => {
  ...
  return (
    <div className="space-y-6" dir="rtl">
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/20 text-xs font-black backdrop-blur-md">
              ...
              <span>المنتج الممتاز المخصص: مراجعات التحصيلي</span>
            </div>
            <h1 className="text-2xl sm:text-3xl font-black">إدارة قسم مراجعات التحصيلي 🎓</h1>
            <p className="text-white/80 font-bold text-xs sm:text-sm max-w-xl leading-relaxed">
              قم بإنشاء وتعديل مراجعات التحصيلي المدفوعة للطلاب. المراجعات تظهر للطلاب كمنتج متميز منفصل لمساعدتهم على اجتياز اختبار التحصيلي بتفوق.
            </p>
          </div>
          <button
      ...
      <div className="bg-white dark:bg-[#111827] p-4 rounded-2xl border border-gray-150 dark:border-slate-800 flex flex-col sm:flex-row gap-4 items-center justify-between">
        <div className="relative w-full sm:w-72">
          ...
          <input
            ...
            className="w-full pl-4 pr-10 py-2.5 bg-gray-50 dark:bg-[#090D16] border border-gray-200 dark:border-slate-800 rounded-xl text-xs font-bold text-gray-800 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all"
          />
        </div>
              ...
              <button
                ...
                className={`px-4 py-2 rounded-xl text-xs font-black transition-all shrink-0 cursor-pointer border-0 ${
                  isActive 
                    ...
                    : 'bg-gray-50 dark:bg-[#090D16] text-gray-500 hover:bg-gray-100 dark:hover:bg-[#222] hover:text-gray-900 dark:hover:text-white'
                }`}
              >
                {labels[filter]}
              </button>
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3].map((n) => (
            <div key={n} className="bg-white dark:bg-[#111827] rounded-2xl border border-gray-150 dark:border-slate-800 p-5 h-72 animate-pulse space-y-4">
        ...
        <div className="bg-white dark:bg-[#111827] rounded-3xl p-12 text-center border border-gray-150 dark:border-slate-800 shadow-sm max-w-md mx-auto space-y-4">
          ...
          <h3 className="text-lg font-black text-gray-900 dark:text-white">لا توجد مراجعات تحصيلي بعد</h3>
          <p className="text-xs text-gray-500 dark:text-gray-400 font-bold max-w-xs mx-auto leading-relaxed">
            لم تقم بإنشاء مراجعات تحصيلي مطابقة لشروط الفلترة الحالية. ابدأ الآن بإنشاء أول فيديو مراجعة مدفوع ومتميز!
          </p>
          <button
            onClick={handleOpenCreate}
          ...
          {filteredReviews.map((review) => {
            ...
            return (
              <motion.div
                key={review.id}
                layoutId={review.id}
                className="bg-white dark:bg-[#111827] border border-gray-150 dark:border-slate-800 rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-all flex flex-col group relative"
              >
                {/* Image & Badges */}
                ...
                <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
                  ...
                  {/* Stat Bar */}
                  <div className="flex items-center justify-between text-[11px] text-gray-500 dark:text-gray-400 font-bold border-t border-b border-gray-100 dark:border-slate-800 py-2.5">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <span className="text-[9px] text-gray-400 font-bold block">مراجعة تحصيلي مدفوعة</span>
                    </div>
        ...
        {showModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm overflow-y-auto">
            <motion.div
              ...
              className="bg-white dark:bg-[#111827] border border-gray-200 dark:border-slate-800 rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden my-8"
            >
              {/* Modal Header */}
              <div className="p-6 border-b border-gray-150 dark:border-slate-800 flex items-center justify-between bg-gradient-to-r from-purple-500/10 to-indigo-500/10">
                <div className="flex items-center gap-2">
                  <div className="w-10 h-10 rounded-xl bg-purple-600 text-white flex items-center justify-center">
                    <Film className="w-5 h-5" />
                  </div>
                  <h3 className="text-lg font-black text-gray-900 dark:text-white">
                    {editingReview ? 'تعديل مراجعة التحصيلي' : 'إنشاء مراجعة تحصيلي جديدة'}
                  </h3>
                </div>
                <button
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[70vh] overflow-y-auto custom-scrollbar text-right">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1.5 sm:col-span-2">
                    ...
                    <div className="flex flex-wrap gap-3">
                      <button
                        ...
                        className={`px-4 py-2.5 rounded-xl border text-xs font-bold transition-all ${contentType === 'video_course' ? 'bg-purple-50 dark:bg-purple-900/20 border-purple-500 text-purple-700 dark:text-purple-400' : 'bg-white dark:bg-[#111827] border-gray-200 dark:border-slate-800 text-gray-600 dark:text-gray-400'}`}
                      >
                      ...
                      <button
                        ...
                        className={`px-4 py-2.5 rounded-xl border text-xs font-bold transition-all ${contentType === 'pdf_book' ? 'bg-rose-50 dark:bg-rose-900/20 border-rose-500 text-rose-700 dark:text-rose-400' : 'bg-white dark:bg-[#111827] border-gray-200 dark:border-slate-800 text-gray-600 dark:text-gray-400'}`}
                      >
                      ...
                      <button
                        ...
                        className={`px-4 py-2.5 rounded-xl border text-xs font-bold transition-all ${contentType === 'exam' ? 'bg-emerald-50 dark:bg-emerald-900/20 border-emerald-500 text-emerald-700 dark:text-emerald-400' : 'bg-white dark:bg-[#111827] border-gray-200 dark:border-slate-800 text-gray-600 dark:text-gray-400'}`}
                      >
                  ...
                  <div className="sm:col-span-2 space-y-1.5">
                    <label className="text-xs font-black text-gray-700 dark:text-gray-300">عنوان المراجعة (مثال: الشامل في فيزياء التحصيلي):</label>
                    <input
                      type="text"
                      required
                      value={title}
                      onChange={(e) => setTitle(e.target.value)}
                      className="w-full px-4 py-3 bg-gray-50 dark:bg-[#090D16] border border-gray-200 dark:border-slate-800 rounded-2xl text-xs font-bold focus:outline-none focus:ring-2 focus:ring-purple-500"
                    />
                  </div>
                  ...
                  <div className="sm:col-span-2 space-y-1.5">
                    ...
                    <textarea
                      ...
                      className="w-full px-4 py-3 bg-gray-50 dark:bg-[#090D16] border border-gray-200 dark:border-slate-800 rounded-2xl text-xs font-bold focus:outline-none focus:ring-2 focus:ring-purple-500 resize-none"
                    />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="bg-gray-50 dark:bg-[#090D16] border border-gray-250 dark:border-slate-800 p-3 rounded-2xl space-y-2">
                      ...
                      <div className="relative border border-dashed border-gray-300 dark:border-slate-800 rounded-xl p-4 flex flex-col items-center justify-center text-center hover:bg-gray-100/50 dark:hover:bg-[#151520] transition-colors cursor-pointer min-h-[100px]">
                        <input
                          type="file"
                          accept="image/*"
                          onChange={(e) => {
                  ...
                  <div className="sm:col-span-2 space-y-2">
                    ...
                    <div className="bg-gray-50 dark:bg-[#090D16] border border-gray-250 dark:border-slate-800 p-4 rounded-3xl space-y-3">
                      <div className="flex gap-2 border-b border-gray-150 dark:border-[#20202C] pb-2">
                        <button
                          type="button"
                      ...
                      <div className="relative border border-dashed border-gray-350 dark:border-slate-800 rounded-2xl p-6 flex flex-col items-center justify-center text-center hover:bg-gray-100/50 dark:hover:bg-[#151520] transition-colors cursor-pointer min-h-[120px]">
                        <input
                          type="file"
                          accept="video/*"
                          onChange={(e) => {
                      ...
                      <div className="space-y-1.5 pt-2 border-t border-gray-150 dark:border-[#20202C]">
                        ...
                        <input
                          ...
                          className="w-full px-4 py-2.5 bg-white dark:bg-[#111827] border border-gray-200 dark:border-slate-800 rounded-2xl text-xs font-bold focus:outline-none focus:ring-1 focus:ring-purple-500"
                        />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="sm:col-span-2 border border-gray-150 dark:border-slate-800 bg-gray-50/50 dark:bg-[#090D16]/30 rounded-3xl p-5 space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="relative border border-dashed border-gray-300 dark:border-slate-800 rounded-2xl p-4 flex flex-col items-center justify-center text-center hover:bg-gray-100/50 dark:hover:bg-[#151520] transition-colors cursor-pointer min-h-[120px] bg-white dark:bg-[#111827]">
                          <input
                            type="file"
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="bg-white dark:bg-[#111827] border border-gray-200 dark:border-slate-800 rounded-2xl p-4 space-y-3 min-h-[120px] flex flex-col justify-center">
                          <select
                            value={examId}
                            onChange={(e) => setExamId(e.target.value)}
                            className="w-full px-3 py-2.5 bg-gray-50 dark:bg-[#090D16] border border-gray-200 dark:border-slate-800 rounded-xl text-xs font-bold focus:outline-none focus:ring-2 focus:ring-purple-500 cursor-pointer"
                          >
                          ...
                          <div className="relative flex items-center gap-2 pt-2 border-t border-gray-100 dark:border-slate-800">
                            ...
                            <input
                              ...
                              value={examId}
                              onChange={(e) => setExamId(e.target.value)}
                              className="flex-1 px-3 py-1.5 bg-gray-50 dark:bg-[#090D16] border border-gray-200 dark:border-slate-800 rounded-lg text-[10px] font-mono focus:outline-none focus:ring-1 focus:ring-purple-500"
                            />
                          </div>
                        </div>
                      </div>
                ...
                {/* Footer buttons */}
                <div className="flex gap-3 pt-6 border-t border-gray-150 dark:border-slate-800 mt-6">
                  <button
                    type="submit"
                    disabled={submitting}
        ...
        {deletingReviewId && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 20 }}
              className="bg-white dark:bg-[#111827] border border-gray-200 dark:border-slate-800 rounded-3xl shadow-2xl w-full max-w-md overflow-hidden p-6 text-right"
              dir="rtl"
            >
              <div className="flex flex-col items-center text-center space-y-4">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 font-bold leading-relaxed">
                  هل أنت متأكد تماماً من رغبتك في حذف مراجعة التحصيلي هذه نهائياً؟ هذا الإجراء لا يمكن التراجع عنه وسيلغي اشتراك جميع الطلاب المسجلين بها فوراً.
                </p>
              </div>
