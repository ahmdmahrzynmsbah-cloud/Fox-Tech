...
export default function StudentStore({ userData, setUserData }: { userData: any, setUserData: (data: any) => void }) {
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white dark:bg-[#111827] rounded-3xl p-6 md:p-8 border border-gray-200 dark:border-slate-800 shadow-sm">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 bg-[#D4F800]/15 text-[#658C00] dark:bg-[#D4F800]/20 dark:text-[#D4F800] border border-[#D4F800]/30 rounded-2xl flex items-center justify-center shadow-md">
          ...
          <div className="flex items-center gap-3 bg-gray-50 dark:bg-[#0D121F] p-3 rounded-2xl border border-slate-200 dark:border-white/10">
            <div className="w-10 h-10 bg-[#D4F800]/15 text-[#658C00] dark:bg-[#D4F800]/20 dark:text-[#D4F800] rounded-xl flex items-center justify-center">
              <Ticket className="w-5 h-5" />
            </div>
            <div>
              <span className="block text-xs font-bold text-gray-500">الرصيد المتاح</span>
              <span className="block text-sm font-black text-[#658C00] dark:text-[#D4F800] font-mono">{userData?.balance || 0} ج.م</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-4 scrollbar-none mb-6">
          <button
            ...
            className={`shrink-0 px-5 py-2.5 rounded-xl text-sm font-black transition-all cursor-pointer ${
              selectedCat === 'all'
                ? 'bg-[#D4F800] text-[#0A102E] shadow-md'
                : 'bg-gray-100 dark:bg-[#1E2433]/60 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-[#1E2433]'
            }`}
          >
            الكل
          </button>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredItems.map(item => {
            ...
            return (
              <div key={item.id} className="bg-white dark:bg-[#0D121F] border border-slate-200 dark:border-white/10 rounded-3xl overflow-hidden hover:shadow-lg transition-all group flex flex-col">
                <div className="h-48 bg-gray-100 dark:bg-[#090D16] relative overflow-hidden">
                  ...
                  {isPurchased && (
                    <div className="absolute inset-0 bg-[#0A102E]/60 backdrop-blur-[2px] flex items-center justify-center z-10">
                      <div className="bg-[#D4F800] text-[#0A102E] px-4 py-2 rounded-xl font-black transform -rotate-12 shadow-xl flex items-center gap-2">
                ...
                <div className="p-5 flex flex-col flex-1">
                  ...
                  <div className="flex items-center justify-between mt-auto pt-4 border-t border-gray-100 dark:border-slate-800">
                    <span className="text-lg font-black text-[#658C00] dark:text-[#D4F800] font-mono">{item.sellingPrice} ج.م</span>
                    <button
                      onClick={() => setSelectedItem(item)}
                      disabled={item.stock <= 0 || isPurchased}
                      className={`px-4 py-2 rounded-xl text-sm font-black transition-colors shadow-md cursor-pointer ${isPurchased ? 'bg-slate-100 dark:bg-slate-800 text-gray-400 cursor-not-allowed shadow-none' : item.stock <= 0 ? 'bg-gray-300 dark:bg-[#2D2D3D] text-gray-500 cursor-not-allowed shadow-none' : 'bg-[#D4F800] hover:bg-[#c2e400] text-[#0A102E]'}`}
                    >
        ...
        {filteredItems.length === 0 && (
          <div className="py-20 flex flex-col items-center justify-center text-center">
            <div className="w-20 h-20 bg-gray-50 dark:bg-[#0D121F] rounded-full flex items-center justify-center mb-4">
      ...
      {selectedItem && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white dark:bg-[#111827] rounded-3xl p-6 md:p-8 max-w-md w-full shadow-2xl relative border border-slate-200 dark:border-white/10">
            <div className="text-center">
              <div className="w-16 h-16 bg-[#D4F800]/20 text-[#658C00] dark:text-[#D4F800] border border-[#D4F800]/40 rounded-full flex items-center justify-center mx-auto mb-4 shadow-md">
              ...
              <div className="bg-gray-50 dark:bg-[#0D121F] rounded-2xl p-4 mb-6 text-right">
                ...
                <div className="flex justify-between items-center mb-3 text-sm">
                  ...
                  <span className="font-black text-[#658C00] dark:text-[#D4F800] font-mono">{userData?.balance || 0} ج.م</span>
                </div>
                <div className="flex justify-between items-center pt-3 border-t border-gray-200 dark:border-slate-800 text-sm">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 px-4 py-3 bg-gray-100 hover:bg-gray-200 dark:bg-[#2D2D3D] dark:hover:bg-[#3D3D52] text-gray-700 dark:text-white rounded-xl font-black transition-colors text-sm cursor-pointer"
                >
                  إلغاء
                </button>
                <button
                  ...
                  className="flex-1 px-4 py-3 bg-[#D4F800] hover:bg-[#c2e400] text-[#0A102E] disabled:bg-gray-300 dark:disabled:bg-[#2D2D3D] disabled:text-gray-500 disabled:cursor-not-allowed rounded-xl font-black transition-colors shadow-md text-sm flex items-center justify-center gap-2 cursor-pointer"
                >
