    ...
    >
      ...
      {isLoading && (
        <div className="absolute inset-0 bg-[#0A0A10] flex flex-col items-center justify-center gap-3 z-30 animate-pulse">
          <Loader2 className="w-10 h-10 animate-spin text-sky-600 dark:text-cyan-400" />
      ...
      {!isPlaying && !isLoading && !error && (
        <button
          onClick={togglePlay}
          className="absolute w-16 h-16 sm:w-20 sm:h-20 bg-gradient-to-r from-cyan-500 to-indigo-600 dark:from-cyan-400 dark:to-indigo-500 text-white rounded-full flex items-center justify-center shadow-2xl hover:scale-110 active:scale-95 transition-all cursor-pointer z-20 animate-bounce-slow"
