...
import { 
  User, Phone, Mail, MapPin, School, GraduationCap, Laptop, Cpu,
...
export default function ProfileSection({ userData, onUpdateUserData }: ProfileSectionProps) {
  ...
  const [avatarBg, setAvatarBg] = useState(userData?.avatarBg || 'from-[#0A102E] via-[#182672] to-[#0D1540]');
  const [isFlipped, setIsFlipped] = useState(false);
  const [generatedAdminCode, setGeneratedAdminCode] = useState('');
  const [generatingCode, setGeneratingCode] = useState(false);

  // Helper to convert Tailwind gradient class into standard CSS linear-gradient for exact print fidelity
  const getGradientCss = (bgClass: string) => {
    if (bgClass.includes('0A102E') || bgClass.includes('182672')) {
      return 'linear-gradient(135deg, #0A102E 0%, #182672 50%, #0D1540 100%)';
    }
    if (bgClass.includes('00B4D8') || bgClass.includes('0077B6')) {
      return 'linear-gradient(135deg, #00B4D8 0%, #0077B6 100%)';
    }
    if (bgClass.includes('D4AF37') || bgClass.includes('B8860B')) {
      return 'linear-gradient(135deg, #D4AF37 0%, #B8860B 100%)';
    }
    if (bgClass.includes('7209B7') && bgClass.includes('3F37C9')) {
      return 'linear-gradient(135deg, #7209B7 0%, #3F37C9 100%)';
    }
    if (bgClass.includes('FF007F')) {
      return 'linear-gradient(135deg, #FF007F 0%, #7209B7 100%)';
    }
    if (bgClass.includes('4CCC81')) {
      return 'linear-gradient(135deg, #4CCC81 0%, #2E8B57 100%)';
    }
    if (bgClass.includes('FF6B6B')) {
  ...
  // Student & User Status Indicator helper
  const statusInfo = (() => {
    const rawStatus = (userData?.status || '').toLowerCase().trim();
    const isApproved = userData?.isApproved !== false;

    if (rawStatus === 'suspended' || rawStatus === 'معلق' || rawStatus === 'pending' || !isApproved) {
      return {
        status: 'suspended',
        label: 'معلق',
        fullLabel: 'حساب معلق',
        shortLabel: 'معلق',
        badgeBg: 'bg-amber-500/25 text-amber-200 border-amber-400/40 shadow-amber-500/20',
        dotColor: 'bg-amber-400',
        glowColor: 'shadow-[0_0_8px_#fbbf24]',
        pingColor: 'bg-amber-400',
        textColor: 'text-amber-300',
      };
    }

    if (rawStatus === 'expired' || rawStatus === 'منتهي' || rawStatus === 'inactive' || rawStatus === 'موقوف') {
      return {
        status: 'expired',
        label: 'منتهي',
        fullLabel: 'اشتراك منتهي',
        shortLabel: 'منتهي',
        badgeBg: 'bg-rose-500/25 text-rose-200 border-rose-400/40 shadow-rose-500/20',
        dotColor: 'bg-rose-400',
        glowColor: 'shadow-[0_0_8px_#f43f5e]',
    ...
    return {
      ...
      fullLabel: 'حساب نشط',
      shortLabel: 'نشط',
      badgeBg: 'bg-emerald-500/25 text-emerald-200 border-emerald-400/40 shadow-emerald-500/20',
      dotColor: 'bg-emerald-400',
      glowColor: 'shadow-[0_0_8px_#34d399]',
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      {/* Profile Header Card */}
      <div className="bg-white dark:bg-[#111827] rounded-3xl p-6 sm:p-8 border border-gray-200 dark:border-slate-800 shadow-sm flex flex-col md:flex-row items-center gap-6 relative overflow-hidden">
        <div className="absolute top-0 left-0 w-32 h-32 bg-sky-500/5 dark:bg-cyan-400/5 rounded-full blur-2xl"></div>
        <div className={`w-24 h-24 bg-gradient-to-br ${avatarBg} rounded-full border-4 border-white dark:border-slate-800 flex items-center justify-center font-black text-4xl text-white shadow-lg shrink-0 relative transition-all duration-500`}>
          {name.charAt(0) || 'U'}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="bg-white dark:bg-[#111827] rounded-2xl p-3 border border-gray-200 dark:border-slate-800 shadow-sm flex md:flex-col gap-1 overflow-x-auto md:overflow-visible sticky top-24 z-10">
            ...
            <>
              <button
                ...
                className={`flex-1 md:flex-none flex items-center justify-center md:justify-start gap-3 px-4 py-3 rounded-xl font-bold text-sm transition-all whitespace-nowrap ${
                  activeSubTab === 'stats'
                    ? 'bg-sky-500/10 dark:bg-cyan-400/10 text-sky-600 dark:text-cyan-400'
              ...
              >
                <BookOpen className="w-4 h-4 shrink-0" /> {userData?.role === 'teacher' ? 'المسارات المنشورة' : 'المسارات التدريبية'}
              </button>
          ...
          >
            {userData?.role === 'teacher' ? (
              <>
                <Laptop className="w-4 h-4 shrink-0" /> بيانات التدريب والتأهيل
              </>
            ) : userData?.role === 'parent' ? (
              <>
                <Users className="w-4 h-4 shrink-0" /> متابعة المتدربين
              </>
            ) : userData?.role === 'admin' ? (
              <>
              ...
              <>
                <Laptop className="w-4 h-4 shrink-0" /> بيانات المسار والتدريب
              </>
            )}
          </button>
        ...
        <div className="md:col-span-3">
          <AnimatePresence mode="wait">
            ...
            {activeSubTab === 'stats' && (
              ...
              >
                {loadingStatsData ? (
                  <div className="bg-white dark:bg-[#111827] rounded-3xl p-12 border border-gray-200 dark:border-slate-800 shadow-sm flex flex-col items-center justify-center space-y-4">
                    <div className="w-10 h-10 border-4 border-sky-500 dark:border-cyan-400 border-t-transparent rounded-full animate-spin"></div>
                  ...
                  <>
                    ...
                    {/* Chart Card */}
                    <div className="bg-white dark:bg-[#111827] rounded-3xl p-6 sm:p-8 border border-gray-200 dark:border-slate-800 shadow-sm space-y-6">
            ...
            {activeSubTab === 'courses' && (
              <motion.div
                ...
                className="space-y-6"
              >
                <div className="bg-white dark:bg-[#111827] rounded-3xl p-6 sm:p-8 border border-gray-200 dark:border-slate-800 shadow-sm">
                  <div className="mb-6">
                    <h3 className="text-lg font-black text-gray-900 dark:text-white mb-1">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                      {myCourses.map((course) => {
                        ...
                        return (
                          <div 
                            key={course.id}
                            className="bg-gray-50 dark:bg-[#090D16] rounded-2xl overflow-hidden border border-gray-150 dark:border-slate-800 flex flex-col justify-between group hover:shadow-md transition-all h-full"
                          >
                            <div className="p-4 space-y-3">
                              {/* Subject / Grade Tags */}
                              <div className="flex items-center justify-between">
                                <span className="bg-sky-500/10 dark:bg-cyan-400/10 text-sky-600 dark:text-cyan-400 px-2.5 py-0.5 rounded-lg text-[10px] font-black">
                                  {course.subject}
                                </span>
                              ...
                              {/* Title */}
                              <h4 className="font-black text-sm text-gray-900 dark:text-white group-hover:text-sky-600 dark:group-hover:text-cyan-400 transition-colors line-clamp-1">
                                {course.title}
                              </h4>
                              ...
                              {userData?.role === 'student' && (
                                <div className="space-y-1.5 pt-2">
                                  <div className="flex justify-between items-center text-[10px] font-bold">
                                    ...
                                    <span className="text-sky-600 dark:text-cyan-400 font-sans">{percent}%</span>
                                  </div>
                                  <div className="w-full bg-gray-200 dark:bg-[#2D2D3D] h-1.5 rounded-full overflow-hidden">
                                    <div 
                                      className="bg-gradient-to-r from-cyan-500 to-indigo-600 dark:from-cyan-400 dark:to-indigo-500 h-full rounded-full transition-all duration-300"
                                      style={{ width: `${percent}%` }}
                                    />
                                  </div>
                              ...
                              {userData?.role === 'teacher' && (
                                <div className="grid grid-cols-2 gap-2 pt-2 text-[10px] font-bold text-gray-500 dark:text-gray-400">
                                  <div className="bg-white dark:bg-[#111827] p-2 rounded-xl border border-gray-150 dark:border-slate-800 text-center">
                            ...
                            <div className="p-4 border-t border-gray-150 dark:border-slate-800 bg-white dark:bg-[#151520] flex items-center justify-between">
                              <span className="text-xs font-black text-sky-600 dark:text-cyan-400">
                      ...
                      {[
                        {
                          ...
                          status: myCourses.length > 0 ? `تم نشر ${myCourses.length} مسار تدريبي` : 'لم يتم نشر مسارات بعد'
                        },
                        {
                          id: 'teacher-reach',
                        ...
                        <div 
                          ...
                          className={`p-4 rounded-2xl border flex items-center gap-4 transition-all duration-300 ${
                            badge.unlocked 
                              ...
                              : 'bg-gray-50 dark:bg-[#090D16]/50 border-gray-150 dark:border-slate-800 opacity-40'
                          }`}
                        >
                          <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-black text-xl shrink-0 ${
                            badge.unlocked ? 'bg-white/90 dark:bg-[#111827]/90 shadow-sm' : 'bg-gray-200 dark:bg-[#2D2D3D] text-gray-400'
                          }`}>
            ...
            {activeSubTab === 'basic' && (
              <motion.div
                ...
                animate={{ opacity: 1, x: 0 }}
                exit={{ opacity: 0, x: 10 }}
                className="bg-white dark:bg-[#111827] rounded-3xl p-6 sm:p-8 border border-gray-200 dark:border-slate-800 shadow-sm space-y-6"
              >
                <div>
                ...
                <form onSubmit={handleSaveProfile} className="space-y-4">
                  ...
                  <div className="p-4 bg-gray-50 dark:bg-[#090D16] rounded-2xl border border-gray-100 dark:border-slate-800 space-y-3">
                    <label className="text-xs font-black text-gray-500 dark:text-gray-400 flex items-center gap-1.5">
                      <Sparkles className="w-3.5 h-3.5 text-sky-600 dark:text-cyan-400" /> لون مظهر الحساب (الخلفية)
                    </label>
                    <div className="flex flex-wrap gap-2.5">
                      {[
                        { name: 'كحلي Fox Tech الرسمي', value: 'from-[#0A102E] via-[#182672] to-[#0D1540]' },
                        { name: 'سماوي تقني', value: 'from-[#00B4D8] to-[#0077B6]' },
                        ...
                        <button
                          ...
                          className={`w-10 h-10 rounded-full bg-gradient-to-br ${item.value} relative border-2 transition-transform duration-200 active:scale-95 ${
                            avatarBg === item.value 
                              ? 'border-sky-500 dark:border-cyan-400 scale-110 shadow-md shadow-sky-500/20' 
                              : 'border-transparent'
                          }`}
                          title={item.name}
                        >
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-black text-gray-500 dark:text-gray-400 flex items-center gap-1.5">
                      <User className="w-3.5 h-3.5 text-sky-600 dark:text-cyan-400" /> الاسم كاملاً
                    </label>
                    <input
                      type="text"
                      required
                      ...
                      className="w-full bg-gray-50 dark:bg-[#090D16] border border-gray-200 dark:border-slate-800 focus:border-sky-500 dark:focus:border-cyan-500 focus:bg-white dark:focus:bg-[#1A1A24] rounded-xl px-4 py-3 text-sm text-gray-900 dark:text-white outline-none transition-colors"
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-black text-gray-500 dark:text-gray-400 flex items-center gap-1.5">
                      <Phone className="w-3.5 h-3.5 text-sky-600 dark:text-cyan-400" /> رقم الهاتف (واتساب)
                    </label>
                    <input
                      type="tel"
                      required
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-black text-gray-500 dark:text-gray-400 flex items-center gap-1.5">
                      <MapPin className="w-3.5 h-3.5 text-sky-600 dark:text-cyan-400" /> المحافظة
                    </label>
                    <select
                      value={governorate}
                  ...
                  <div className="pt-4">
                    <button
                      type="submit"
                      disabled={loading}
                      className="w-full bg-sky-600 dark:bg-cyan-500 hover:bg-sky-700 dark:hover:bg-cyan-600 disabled:bg-gray-300 dark:disabled:bg-gray-800 text-white font-bold py-3.5 px-6 rounded-xl shadow-lg shadow-sky-500/10 dark:shadow-cyan-500/10 flex items-center justify-center gap-2 transition-all"
                    >
                      {loading ? (
            ...
            {activeSubTab === 'academic' && (
              ...
              >
                <div>
                  <h3 className="text-lg font-black text-gray-900 dark:text-white mb-1">
                    {userData?.role === 'teacher' ? 'بيانات التدريب والتأهيل' : userData?.role === 'parent' ? 'توصيل ومتابعة المتدربين' : userData?.role === 'admin' ? 'التحكم الإداري والنظام' : 'المسار التدريبي والبيانات المهنية'}
                  </h3>
                ...
                <form onSubmit={handleSaveProfile} className="space-y-4">
                  {userData?.role === 'student' && (
                    <>
                      <div className="space-y-2">
                        <label className="text-xs font-black text-gray-500 dark:text-gray-400 flex items-center gap-1.5">
                          <Laptop className="w-3.5 h-3.5 text-sky-600 dark:text-cyan-400" /> المسار التدريبي الأساسي
                        </label>
                        <select
                          required
                          value={grade}
                          ...
                          className="w-full bg-gray-50 dark:bg-[#090D16] border border-gray-200 dark:border-slate-800 focus:border-sky-500 dark:focus:border-cyan-500 focus:bg-white dark:focus:bg-[#1A1A24] rounded-xl px-4 py-3 text-sm text-gray-900 dark:text-white outline-none transition-colors"
                        >
                          <option value="مسار الـ Frontend (تطوير الواجهات وتطبيقات الويب)">مسار الـ Frontend (تطوير الواجهات وتطبيقات الويب)</option>
                          <option value="مسار الـ Backend (هندسة الخوادم وقواعد البيانات)">مسار الـ Backend (هندسة الخوادم وقواعد البيانات)</option>
                          <option value="مسار الـ Full-Stack (الأنظمة السحابية والمشاريع المتكاملة)">مسار الـ Full-Stack (الأنظمة السحابية والمشاريع المتكاملة)</option>
                          <option value="مسار الذكاء الاصطناعي وهندسة البيانات (AI & Data)">مسار الذكاء الاصطناعي وهندسة البيانات (AI & Data)</option>
                          <option value="مسار تطوير تطبيقات الهواتف (Mobile Apps)">مسار تطوير تطبيقات الهواتف (Mobile Apps)</option>
                          <option value="مسار الأمن السيبراني وهندسة السحابة (DevOps)">مسار الأمن السيبراني وهندسة السحابة (DevOps)</option>
                        </select>
                      </div>
                    </>
                  )}
                  ...
                  {userData?.role === 'teacher' && (
                    <>
                      <div className="space-y-2">
                        <label className="text-xs font-black text-gray-500 dark:text-gray-400 flex items-center gap-1.5">
                          <BookOpen className="w-3.5 h-3.5 text-sky-600 dark:text-cyan-400" /> المادة الأساسية التي تقوم بتدريسها
                        </label>
                        <input
                          type="text"
                          required
                          ...
                          className="w-full bg-gray-100 dark:bg-[#111827] border border-gray-200 dark:border-slate-800 rounded-xl px-4 py-3 text-sm text-gray-500 dark:text-gray-400 outline-none cursor-not-allowed opacity-70"
                      ...
                      <div className="space-y-2">
                        <label className="text-xs font-black text-gray-500 dark:text-gray-400 flex items-center gap-1.5">
                          <IdCard className="w-3.5 h-3.5 text-sky-600 dark:text-cyan-400" /> الرقم القومي المكون من 14 رقم (للتوثيق)
                        </label>
                        <input
                          type="text"
                          maxLength={14}
                      ...
                      <div className="space-y-2">
                        <label className="text-xs font-black text-gray-500 dark:text-gray-400 flex items-center gap-1.5">
                          <Calendar className="w-3.5 h-3.5 text-sky-600 dark:text-cyan-400" /> تاريخ الميلاد
                        </label>
                        <input
                          type="date"
                          required
                          ...
                          className="w-full bg-gray-50 dark:bg-[#090D16] border border-gray-200 dark:border-slate-800 focus:border-sky-500 dark:focus:border-cyan-500 focus:bg-white dark:focus:bg-[#1A1A24] rounded-xl px-4 py-3 text-sm text-gray-900 dark:text-white outline-none transition-colors text-right"
                        />
                      </div>
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                          {[
                            ...
                            const isChecked = teachingGrades.includes(g.value);
                            return (
                              <button
                                key={g.value}
                                type="button"
                                ...
                                className={`p-3.5 rounded-xl border text-sm font-bold transition-all flex items-center justify-center gap-2 ${
                                  isChecked
                                    ? 'bg-sky-500/10 dark:bg-cyan-400/10 border-sky-500 dark:border-cyan-400 text-sky-600 dark:text-cyan-400'
                                    : 'border-gray-200 dark:border-slate-800 text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-[#0D0D12]'
                                }`}
                              >
                                {isChecked && <Check className="w-4 h-4 text-sky-600 dark:text-cyan-400" />}
                                {g.label}
                              </button>
                            );
                          })}
                  ...
                  {userData?.role === 'admin' && (
                    <div className="p-6 bg-gray-50 dark:bg-[#222230]/50 rounded-2xl border border-gray-100 dark:border-slate-800 space-y-4">
                      ...
                      {generatedAdminCode && (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2">
                            <code className="flex-1 block text-left bg-white dark:bg-[#090D16] px-3 py-2 rounded-lg font-mono text-sm border border-green-100 dark:border-green-900/50 text-gray-900 dark:text-gray-100">
                              {generatedAdminCode}
                            </code>
                            <button
                              type="button"
                  ...
                  {userData?.role === 'parent' && (
                    <>
                      <div className="space-y-2">
                        <label className="text-xs font-black text-gray-500 dark:text-gray-400 flex items-center gap-1.5">
                          <Phone className="w-3.5 h-3.5 text-sky-600 dark:text-cyan-400" /> رقم هاتف الطالب المرتبط بحسابك لمتابعة أدائه
                        </label>
                        <input
                          type="tel"
                          required
            ...
            {activeSubTab === 'idCard' && (
              ...
              >
                <div className="bg-white dark:bg-[#111827] rounded-3xl p-6 sm:p-8 border border-gray-200 dark:border-slate-800 shadow-sm">
                  <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-gray-100 dark:border-slate-800/80">
                    <div>
                    ...
                    <div className="flex items-center gap-2 self-start sm:self-auto bg-gray-50 dark:bg-[#0F1117]/60 px-3 py-1.5 rounded-2xl border border-gray-200 dark:border-slate-800">
                      <span className="text-xs text-gray-500 dark:text-gray-400 font-bold">حالة الطالب:</span>
                      <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-black border ${statusInfo.badgeBg}`}>
                        <span className="relative flex h-2 w-2">
                          <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${statusInfo.pingColor}`}></span>
                          <span className={`relative inline-flex rounded-full h-2 w-2 ${statusInfo.dotColor} ${statusInfo.glowColor}`}></span>
                        </span>
                  ...
                  <div className="flex flex-col items-center justify-center py-6">
                    ...
                    >
                      <div className={`relative w-full h-full duration-700 [transform-style:preserve-3d] ${isFlipped ? '[transform:rotateY(180deg)]' : ''}`}>
                        ...
                        <div 
                          ...
                          className={`absolute inset-0 w-full h-full rounded-2xl p-6 text-white shadow-2xl [backface-visibility:hidden] flex flex-col justify-between overflow-hidden border border-white/20`}
                        >
                          {/* Design accents */}
                          ...
                          <div className="flex justify-between items-center z-10 relative gap-2">
                            <div className="flex items-center gap-1.5 flex-wrap">
                              <span 
                                style={{ backgroundColor: 'rgba(255,255,255,0.18)', color: '#ffffff', borderColor: 'rgba(255,255,255,0.3)' }}
                                className="text-[10px] sm:text-xs px-3 py-1 rounded-lg font-black border shadow-xs"
                              >
                              ...
                              {/* Student Status Indicator Badge */}
                              <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[9px] sm:text-[10px] font-black border backdrop-blur-md shadow-xs ${statusInfo.badgeBg}`}>
                                <span className="relative flex h-2 w-2">
                                  <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${statusInfo.pingColor}`}></span>
                                  <span className={`relative inline-flex rounded-full h-2 w-2 ${statusInfo.dotColor} ${statusInfo.glowColor}`}></span>
                                </span>
                            ...
                            <div className="flex items-center gap-2">
                              <FoxTechLogo 
                                ...
                                className="h-8 w-auto max-w-[120px] drop-shadow-md" 
                                variant="dark" 
                              />
                            </div>
                          </div>
                          ...
                          <div className="flex items-center gap-4 my-auto z-10 relative">
                            ...
                            <div className="space-y-1 text-right">
                              <h4 style={{ color: '#ffffff' }} className="font-black text-base truncate max-w-[200px]">{name || 'عضو الأكاديمية'}</h4>
                              <p style={{ color: 'rgba(255,255,255,0.85)' }} className="text-[10px] font-bold tracking-wider">
                                {userData?.role === 'student' ? (grade || 'المسار البرمجي') : userData?.role === 'teacher' ? (subject || 'التخصص التقني') : 'متابع للمتدرب'}
                              </p>
                              <p style={{ color: '#D4F800' }} className="text-[10px] font-mono font-bold tracking-wider">ID: FOX-{userData?.id?.slice(0, 8).toUpperCase() || 'NEW'}</p>
                            </div>
                          </div>

                          {/* Footer */}
                          <div style={{ borderColor: 'rgba(255,255,255,0.2)' }} className="flex justify-between items-end z-10 relative border-t pt-2">
                            <div className="text-right">
                              <p style={{ color: 'rgba(255,255,255,0.6)' }} className="text-[8px]">المحافظة</p>
                              <p style={{ color: '#ffffff' }} className="text-[10px] font-bold">{governorate || 'القاهرة'}</p>
                            </div>
                            <div style={{ color: 'rgba(255,255,255,0.8)' }} className="text-left font-mono text-[9px] tracking-wider">
                              VALID: 2026-2027
                            </div>
                          </div>
                        </div>
                        ...
                        <div 
                          ...
                          className="absolute inset-0 w-full h-full rounded-2xl p-5 sm:p-6 text-white shadow-2xl [backface-visibility:hidden] [transform:rotateY(180deg)] flex flex-col justify-between overflow-hidden border border-white/15"
                        >
                          {/* Magnetic Strip */}
                          <div style={{ backgroundColor: '#020617', borderColor: '#1E293B' }} className="absolute top-3 left-0 right-0 h-8 border-y"></div>

                          {/* Barcode representation */}
                          <div className="mt-6 relative z-10 flex flex-col items-center justify-center w-full">
                            <div style={{ backgroundColor: '#ffffff', color: '#000000', borderColor: '#E2E8F0' }} className="rounded-2xl p-3 sm:p-4 w-full max-w-[340px] flex flex-col items-center justify-center shadow-inner overflow-hidden border">
                          ...
                          {/* Agreement Text */}
                          <div style={{ color: '#94A3B8' }} className="text-[8px] sm:text-[9px] text-center leading-tight font-medium z-10 relative px-2">
                            هذه البطاقة رقمية مشفرة وصادرة من {settings?.platformName || 'أكاديمية Fox Tech'}. الاستخدام يخضع لشروط الخدمة.
                          </div>

                          {/* Contact Info */}
                          <div dir="ltr" style={{ borderColor: 'rgba(255,255,255,0.15)', color: '#CBD5E1' }} className="flex justify-between items-center text-[8px] sm:text-[9.5px] border-t pt-2 z-10 relative font-mono">
                            <div className="flex items-center gap-1.5">
                              <Mail className="w-3 h-3 text-cyan-400 shrink-0" />
                              <span className="truncate max-w-[180px] sm:max-w-[200px]">{settings?.contactEmail || 'fox.tech7777@gmail.com'}</span>
                            </div>
                            <div className="flex items-center gap-1.5">
                    ...
                    <div className="flex gap-3 mt-6 w-full max-w-[400px] print:hidden">
                      <button 
                        ...
                        className="flex-1 flex items-center justify-center gap-2 bg-sky-600 hover:bg-sky-700 text-white font-bold py-2.5 px-4 rounded-xl text-xs transition-all shadow-md shadow-sky-500/20 active:scale-95 cursor-pointer"
                      >
                        <Download className="w-3.5 h-3.5" /> طباعة الهوية الرسمية (PDF)
                      </button>
                      <button 
                        onClick={() => setIsFlipped(!isFlipped)}
                        className="flex-1 flex items-center justify-center gap-2 bg-sky-500/10 dark:bg-cyan-400/10 text-sky-600 dark:text-cyan-400 font-bold py-2.5 px-4 rounded-xl text-xs transition-all active:scale-95 hover:bg-[#00B4D8]/20 dark:hover:bg-[#D4AF37]/20 cursor-pointer"
                      >
            ...
            {activeSubTab === 'security' && (
              ...
              >
                <div className="space-y-4">
                  ...
                  {/* Email Update Form */}
                  <div className="p-4 bg-gray-50 dark:bg-[#090D16] rounded-2xl border border-gray-100 dark:border-slate-800">
                    <h4 className="text-sm font-black text-gray-800 dark:text-gray-200 mb-4 flex items-center gap-2">
                      <Mail className="w-4 h-4 text-sky-600 dark:text-cyan-400" /> تعديل البريد الإلكتروني
                    </h4>
                    <form onSubmit={handleChangeEmail} className="space-y-4">
                      <div className="space-y-2">
                        ...
                        <input
                          ...
                          className="w-full bg-white dark:bg-[#111827] border border-gray-200 dark:border-slate-800 focus:border-sky-500 dark:focus:border-cyan-500 rounded-xl px-4 py-3 text-sm text-gray-900 dark:text-white outline-none transition-colors text-right"
                          dir="ltr"
                  ...
                  <div className="p-4 bg-gray-50 dark:bg-[#090D16] rounded-2xl border border-gray-100 dark:border-slate-800">
                    <h4 className="text-sm font-black text-gray-800 dark:text-gray-200 mb-4 flex items-center gap-2">
                      <Lock className="w-4 h-4 text-sky-600 dark:text-cyan-400" /> تعديل كلمة المرور
                    </h4>
                    <form onSubmit={handleChangePassword} className="space-y-4">
                      <div className="space-y-2">
                        <label className="text-xs font-black text-gray-500 dark:text-gray-400 flex items-center gap-1.5">
                          <Key className="w-3.5 h-3.5 text-sky-600 dark:text-cyan-400" /> كلمة المرور الحالية
                        </label>
                        <input
                          type="password"
                          required
                      ...
                      <div className="space-y-2">
                        <label className="text-xs font-black text-gray-500 dark:text-gray-400 flex items-center gap-1.5">
                          <Lock className="w-3.5 h-3.5 text-sky-600 dark:text-cyan-400" /> كلمة المرور الجديدة
                        </label>
                        <input
                          type="password"
                          required
                      ...
                      <div className="space-y-2">
                        <label className="text-xs font-black text-gray-500 dark:text-gray-400 flex items-center gap-1.5">
                          <ShieldAlert className="w-3.5 h-3.5 text-sky-600 dark:text-cyan-400" /> تأكيد كلمة المرور الجديدة
                        </label>
                        <input
                          type="password"
                          required
                      ...
                      <button
                        type="submit"
                        disabled={loading}
                        className="w-full bg-sky-600 dark:bg-cyan-500 hover:bg-sky-700 dark:hover:bg-cyan-600 disabled:bg-gray-300 dark:disabled:bg-gray-800 text-white font-bold py-3.5 px-6 rounded-xl flex items-center justify-center gap-2 transition-all"
                      >
