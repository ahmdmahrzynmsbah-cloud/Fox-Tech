...
export default function SubscriptionRequests({ adminUserData }: SubscriptionRequestsProps) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      {/* Header card */}
      <div className="bg-white dark:bg-[#111827] p-6 rounded-[2rem] border border-gray-100 dark:border-slate-800 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
      ...
      <div className="bg-white dark:bg-[#111827] p-5 rounded-[2rem] border border-gray-100 dark:border-slate-800 shadow-sm space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="relative md:col-span-1">
            ...
            <input
              ...
              className="w-full pl-4 pr-11 py-3 rounded-2xl bg-gray-50 dark:bg-[#090D16] text-sm text-gray-900 dark:text-white border border-gray-100 dark:border-slate-800/50 focus:outline-none focus:ring-2 focus:ring-sky-500 dark:focus:ring-cyan-400 transition-all font-bold placeholder:text-gray-400"
            />
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex bg-gray-50 dark:bg-[#090D16] p-1 rounded-xl border border-gray-100 dark:border-slate-800/50 w-full">
              {(['pending', 'approved', 'rejected', 'all'] as const).map((status) => (
                <button
                  ...
                  className={`flex-1 py-1.5 text-xs font-black rounded-lg transition-all cursor-pointer ${
                    statusFilter === status
                      ? 'bg-white dark:bg-[#111827] text-gray-900 dark:text-white shadow-sm border border-gray-100 dark:border-slate-800'
      ...
      {loading ? (
        <div className="py-20 flex flex-col items-center justify-center bg-white dark:bg-[#111827] rounded-[2.5rem] border border-gray-100 dark:border-slate-800">
          <RefreshCw className="w-10 h-10 text-sky-600 dark:text-cyan-400 animate-spin mb-4" />
          <p className="text-sm font-black text-gray-400">جاري تحميل طلبات الاشتراك...</p>
        </div>
      ) : filteredRequests.length > 0 ? (
        <div className="bg-white dark:bg-[#111827] rounded-[2rem] border border-gray-100 dark:border-slate-800 shadow-sm overflow-hidden">
                ...
                <AnimatePresence mode="popLayout">
                  {filteredRequests.map((user) => (
                    <motion.tr
                      ...
                      className="border-b border-gray-100 dark:border-slate-800 hover:bg-gray-50/30 dark:hover:bg-[#1e1e2d]/40 transition-colors"
                    >
                      {/* Name with icon avatar */}
                      <td className="py-4 px-6">
                        <div className="flex items-center gap-3">
                          <div className="w-9 h-9 rounded-xl bg-[#00B4D8]/10 text-sky-600 dark:bg-[#D4AF37]/10 dark:text-cyan-400 flex items-center justify-center shrink-0">
        ...
        <div className="py-24 bg-white dark:bg-[#111827] rounded-[3rem] border border-dashed border-gray-200 dark:border-slate-800 text-center">
