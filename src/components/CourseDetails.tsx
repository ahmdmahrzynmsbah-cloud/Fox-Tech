...
export default function CourseDetails() {
    ...
    const fetchData = async (activeUser: any) => {
      ...
      try {
        ...
        if (effectiveUser && effectiveUser.id) {
          ...
          setUserData(effectiveUser as User);
          ...
          try {
            ...
            if (userDoc.exists() && isMounted) {
              ...
              currentUserRole = uData.role || currentUserRole;
              setUserData({ id: userDoc.id, ...uData } as User);
            }
          } catch (e) {
            console.warn("Could not fetch remote user in CourseDetails:", e);
          }
        } else if (!effectiveUser) {
          // If after checking both auth and cache there is no user
          navigate('/login');
          return;
        }
        ...
        if (id) {
          ...
          if (courseDoc.exists()) {
            ...
            if (currentUserId) {
              const likeDoc = await getDoc(doc(db, 'course_likes', `${currentUserId}_${id}`));
              if (isMounted) setIsLiked(likeDoc.exists());
            }
          } else {
            navigate('/dashboard');
            return;
          ...
          let lastWatchedId = "";
          if (currentUserId && id) {
            const progressDoc = await getDoc(doc(db, 'course_progress', `${currentUserId}_${id}`));
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#090D16] text-gray-900 dark:text-white font-sans selection:bg-[#00B4D8]/30 dark:selection:bg-[#D4AF37]/30">
      {/* Header */}
      <header className="bg-white dark:bg-[#111827] border-b border-gray-200 dark:border-slate-800 sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <div className="hidden md:flex items-center gap-2 text-sm font-bold bg-sky-500/10 dark:bg-cyan-400/10 text-sky-600 dark:text-cyan-400 px-4 py-2 rounded-full">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-stretch">
          ...
          <div className="lg:col-span-2 flex flex-col">
            <div className="bg-black rounded-3xl overflow-hidden aspect-video relative shadow-lg border border-gray-200 dark:border-slate-800 w-full flex-1 min-h-[240px] md:min-h-[350px]">
              {!canWatch ? (
                <div className="w-full h-full absolute inset-0 flex flex-col items-center justify-center text-gray-400 dark:text-gray-500 bg-gray-900 p-8 text-center select-none overflow-y-auto">
                  <Lock className="w-16 h-16 mb-4 opacity-50 text-sky-600 dark:text-cyan-400" />
                  ...
                  {isSuspended ? (
                    ...
                    <>
                      ...
                      <button 
                        ...
                        className="px-8 py-3 rounded-xl font-bold text-lg bg-sky-600 dark:bg-cyan-500 text-white dark:text-[#0D0D12] hover:bg-sky-700 dark:hover:bg-cyan-600 transition-colors cursor-pointer border-0"
                      >
                    ...
                    <>
                      ...
                      {course.price && course.price > 0 ? (
                        <button 
                          onClick={() => setShowPaymentModal(true)}
                          className="px-8 py-3 rounded-xl font-bold text-lg bg-sky-600 dark:bg-cyan-500 text-white dark:text-[#0D0D12] hover:bg-sky-700 dark:hover:bg-cyan-600 transition-colors cursor-pointer border-0 animate-pulse"
                        >
                        ...
                        <button 
                          ...
                          disabled={enrolling}
                          className="px-8 py-3 rounded-xl font-bold text-lg bg-sky-600 dark:bg-cyan-500 text-white hover:bg-sky-700 dark:hover:bg-cyan-600 disabled:opacity-50 transition-colors cursor-pointer border-0"
                        >
          ...
          <div className="lg:col-span-1 flex flex-col">
            <div className="bg-white dark:bg-[#111827] rounded-3xl shadow-sm border border-gray-200 dark:border-slate-800 flex flex-col w-full h-full min-h-[350px] lg:min-h-0 lg:h-full overflow-hidden">
              <div className="p-4 border-b border-gray-100 dark:border-slate-800 bg-gray-50/50 dark:bg-[#222230]/50 rounded-t-3xl shrink-0">
                <div className="flex bg-gray-200/50 dark:bg-[#111827] p-1 rounded-xl">
                  <button
                    onClick={() => setContentTab('lessons')}
                    className={`flex-1 py-2 text-xs font-black rounded-lg transition-all cursor-pointer ${contentTab === 'lessons' ? 'bg-white dark:bg-[#2D2D3D] text-sky-600 dark:text-cyan-400 shadow-sm' : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-300'}`}
                  >
              ...
              {userData?.role === 'student' && lessons.length > 0 && (
                <div className="px-5 py-3 bg-gray-50 dark:bg-[#15151F] border-b border-gray-100 dark:border-slate-800 font-sans shrink-0">
                  ...
                  <div className="w-full bg-gray-200 dark:bg-[#2D2D3D] rounded-full h-1.5 overflow-hidden">
                    <motion.div 
                      ...
                      className="h-full bg-gradient-to-r from-cyan-500 to-indigo-600 dark:from-cyan-400 dark:to-indigo-500 rounded-full"
                    />
                  </div>
                </div>
              )}
                ...
                {contentTab === 'lessons' ? (
                    ...
                    lessons.map((lesson, idx) => (
                      <div 
                        ...
                        className={`p-3.5 rounded-xl cursor-pointer transition-all flex items-center gap-3 group border ${
                          ...
                          ? 'bg-sky-500/10 dark:bg-cyan-400/10 border-sky-500/30 dark:border-cyan-400/30 shadow-sm' 
                          : 'hover:bg-gray-50 dark:hover:bg-[#222230] border-transparent'
                        }`}
                      >
                        <div className={`w-7 h-7 rounded-full flex items-center justify-center font-bold text-xs shrink-0 ${
                          activeLesson?.id === lesson.id
                          ? 'bg-sky-600 dark:bg-cyan-500 text-white'
                          : completedLessons.includes(lesson.id)
                          ? 'bg-green-500 text-white'
                          : 'bg-gray-100 dark:bg-[#2D2D3D] text-gray-650 dark:text-gray-300 group-hover:bg-[#00B4D8]/20 dark:group-hover:bg-[#D4AF37]/20 group-hover:text-sky-600 dark:group-hover:text-cyan-400'
                        }`}>
                        ...
                        <div className="flex-1 min-w-0">
                          <h4 className={`font-black truncate text-xs mb-0.5 ${
                            activeLesson?.id === lesson.id ? 'text-sky-700 dark:text-[#B8860B]' : 'text-gray-900 dark:text-white'
                          }`}>
                            {lesson.title}
                          </h4>
                          ...
                          <button
                            ...
                            className={`p-1.5 rounded-xl transition-all self-center shrink-0 cursor-pointer ${
                              completedLessons.includes(lesson.id)
                                ...
                                : 'text-gray-300 dark:text-gray-600 hover:text-sky-600 dark:hover:text-cyan-400 hover:bg-gray-100 dark:hover:bg-[#2D2D3D]'
                            }`}
                  ...
                  0 === 0 ? (
                    ...
                    [].map((stream, idx) => (
                      <div 
                        key={stream.id}
                        className="p-3 bg-gray-50 dark:bg-[#222230] rounded-2xl border border-gray-100 dark:border-slate-800 space-y-2 text-right"
                      >
                        ...
                        {stream.recordingSummary && (
                          <div className="text-[10px] text-gray-600 dark:text-gray-300 bg-white dark:bg-[#111827] p-2.5 rounded-xl border border-gray-100 dark:border-slate-800 leading-relaxed">
                            {stream.recordingSummary}
                          </div>
                        )}
                        
                        {stream.materials && stream.materials.length > 0 && (
                          <div className="pt-1.5 border-t border-gray-100 dark:border-slate-800 flex flex-wrap gap-1">
                            {stream.materials.map((mat: any, mIdx: number) => (
                              <a
                                ...
                                target="_blank"
                                rel="noopener noreferrer"
                                className="flex items-center gap-1 px-2 py-1 bg-white dark:bg-[#2D2D3D] border border-gray-150 dark:border-slate-800 rounded-md hover:border-sky-500 dark:hover:border-cyan-500 transition-all group"
                              >
                                <FileText className="w-2.5 h-2.5 text-gray-400 group-hover:text-sky-600" />
              ...
              {isTeacher && (
                <div className="p-4 border-t border-gray-100 dark:border-slate-800 bg-white dark:bg-[#111827] rounded-b-3xl shrink-0">
                  <button
                    onClick={() => setShowAddLesson(true)}
                    className="w-full py-2.5 rounded-xl border-2 border-dashed border-gray-300 dark:border-slate-800 text-gray-600 dark:text-gray-400 hover:border-sky-500 dark:hover:border-cyan-500 hover:text-sky-600 dark:hover:text-cyan-400 hover:bg-gray-50 dark:hover:bg-[#222230] transition-all flex items-center justify-center gap-2 font-black text-xs cursor-pointer animate-pulse hover:animate-none"
                  >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
          ...
          <div className="lg:col-span-2 space-y-6">
            {activeLesson ? (
              <div className="bg-white dark:bg-[#111827] rounded-3xl p-6 shadow-sm border border-gray-200 dark:border-slate-800 space-y-6">
                {/* Tabs header inside the card */}
                <div className="flex border-b border-gray-100 dark:border-slate-800 pb-3 justify-between items-center">
                  <div className="flex gap-4">
                    ...
                    >
                      وصف الدرس
                      {lessonTab === 'info' && (
                        <motion.div layoutId="activeLessonTabLine" className="absolute bottom-0 left-0 right-0 h-0.5 bg-sky-600 dark:bg-cyan-500" />
                      )}
                    </button>
                ...
                <AnimatePresence mode="wait">
                  {lessonTab === 'info' && (
                    ...
                    >
                      ...
                      {userData?.role === 'student' && (
                        <div className="pt-5 border-t border-gray-100 dark:border-slate-800 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  ...
                  {lessonTab === 'notes' && (
                    <motion.div
                      ...
                      className="space-y-4"
                    >
                      <div className="flex gap-2 bg-gray-50 dark:bg-[#090D16] p-1 rounded-xl w-fit">
                        <button
                          type="button"
                      ...
                      {notesTab === 'current' ? (
                        <div className="space-y-3">
                          ...
                          <textarea
                            ...
                            className="w-full min-h-[150px] bg-gray-50 dark:bg-[#090D16] border border-gray-200 dark:border-slate-800 rounded-xl p-4 text-sm text-gray-900 dark:text-white focus:outline-none focus:border-sky-500 dark:focus:border-cyan-500 focus:bg-white dark:focus:bg-[#1A1A24] transition-colors resize-y leading-relaxed font-medium"
                            dir="rtl"
                          />
                          ...
                          <div className="flex justify-between items-center">
                            ...
                            <button
                              ...
                              className="bg-sky-600 dark:bg-cyan-500 hover:bg-sky-700 dark:hover:bg-cyan-600 disabled:opacity-50 text-white px-5 py-2.5 rounded-xl font-bold text-xs flex items-center gap-1.5 transition-all cursor-pointer"
                            >
                        ...
                        <div className="space-y-4 max-h-[300px] overflow-y-auto">
                          {allNotes.length === 0 ? (
                            ...
                            allNotes.map((note) => (
                              <div key={note.id} className="p-4 rounded-xl bg-gray-50 dark:bg-[#222230] border border-gray-100 dark:border-slate-800 space-y-1.5 text-right relative">
            ...
            <div className="bg-white dark:bg-[#111827] rounded-3xl p-6 shadow-sm border border-gray-200 dark:border-slate-800 space-y-6">
              ...
              <div className="bg-gray-50/50 dark:bg-[#222230]/40 rounded-2xl p-4 border border-gray-100 dark:border-slate-800 flex flex-col md:flex-row items-center justify-between gap-4">
                  ...
                  <motion.button
                    ...
                    className={`px-5 py-2.5 rounded-xl text-xs font-black flex items-center gap-2 transition-all shadow-sm border cursor-pointer ${
                      isLiked 
                        ...
                        : "bg-white dark:bg-[#1C1C26] text-gray-750 dark:text-gray-300 border-gray-200 dark:border-slate-800 hover:bg-gray-50 dark:hover:bg-[#252533]"
                    }`}
                  >
              ...
              {userData?.role === "student" && canWatch && (
                <form onSubmit={handleSubmitReview} className="bg-gray-50 dark:bg-[#222230] p-5 rounded-2xl border border-gray-100 dark:border-slate-800 space-y-4">
                  ...
                  <textarea 
                    ...
                    className="w-full bg-white dark:bg-[#111827] border border-gray-200 dark:border-slate-800 rounded-xl p-3.5 text-xs text-gray-900 dark:text-white focus:outline-none focus:border-sky-500 dark:focus:border-cyan-500 focus:ring-1 focus:ring-sky-500 dark:focus:ring-cyan-400 resize-none leading-relaxed" 
                    rows={3}
                  />
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-1">
                    ...
                    <button 
                      ...
                      className="bg-gradient-to-r from-cyan-500 to-indigo-600 dark:from-cyan-400 dark:to-indigo-500 text-white dark:text-gray-900 px-6 py-2.5 rounded-xl font-black text-xs shadow-md hover:opacity-95 transition-opacity disabled:opacity-50 cursor-pointer flex items-center gap-2"
                    >
                      {isSubmittingReview ? (
                        <>
                        ...
                        <div className="flex-1 space-y-1.5 min-w-0">
                            ...
                            <div className="flex items-center justify-between flex-wrap gap-2 mb-2">
                              <div className="flex items-center gap-2">
                                ...
                                {isMyComment && !isTeacherComment && (
                                  <span className="text-[9px] font-black bg-[#00B4D8]/10 text-sky-600 dark:bg-[#D4AF37]/10 dark:text-cyan-400 px-2 py-0.5 rounded-full">
                                    أنت
                                  </span>
                                )}
                                    ...
                                    <div className="flex-1 bg-gray-50/70 dark:bg-[#1E1E2B]/50 border border-gray-100/30 dark:border-gray-800/40 rounded-xl p-3 shadow-sm min-w-0">
                                      <div className="flex items-center gap-1.5 mb-1 flex-wrap">
                                        ...
                                        {reply.userId === userData?.id && !isReplyTeacher && (
                                          <span className="text-[8px] font-black bg-[#00B4D8]/10 text-sky-600 dark:bg-[#D4AF37]/10 dark:text-cyan-400 px-1.5 py-0.5 rounded-full">
                                            أنت
                                          </span>
                                        )}
                          ...
                          <AnimatePresence>
                            {activeReplyInput === review.id && (
                              ...
                              >
                                <div className="flex gap-2 items-end bg-gray-50 dark:bg-[#1e1e2c] p-2.5 rounded-xl border border-gray-200 dark:border-slate-800">
                                  <textarea 
                                    value={replyText}
                                    onChange={(e) => setReplyText(e.target.value)}
                                    placeholder={`الرد على ${review.userName}...`}
                                    className="flex-1 bg-white dark:bg-[#151520] border border-gray-250 dark:border-gray-800 rounded-lg px-3 py-1.5 text-xs text-gray-950 dark:text-white focus:outline-none focus:border-sky-500 dark:focus:border-cyan-500 resize-none h-10"
                                    rows={1}
                                  />
                                  <button 
                                    onClick={() => handleSubmitReply(review.id)}
                                    disabled={isSubmittingReply[review.id] || !replyText.trim()}
                                    className="bg-sky-600 dark:bg-cyan-500 text-white dark:text-gray-950 p-2 rounded-lg hover:opacity-90 disabled:opacity-40 transition-opacity cursor-pointer flex items-center justify-center flex-shrink-0 w-8 h-8"
                                  >
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            {userData?.role === 'student' && canWatch && (
              <div className="bg-white dark:bg-[#111827] rounded-3xl shadow-sm border border-gray-200 dark:border-slate-800 flex flex-col overflow-hidden">
                <div className="p-4 border-b border-gray-100 dark:border-slate-800 bg-sky-500/5 dark:bg-cyan-400/5 flex items-center justify-between">
                  <h3 className="font-black text-xs text-gray-900 dark:text-white flex items-center gap-2">
                    <Edit2 className="w-4 h-4 text-sky-600 dark:text-cyan-400" />
                ...
                <div className="p-4">
                  <textarea
                    ...
                    className="w-full min-h-[150px] bg-gray-50 dark:bg-[#090D16] border border-gray-200 dark:border-slate-800 rounded-xl p-3 text-xs text-gray-900 dark:text-white focus:outline-none focus:border-sky-500 dark:focus:border-cyan-500 focus:bg-white dark:focus:bg-[#1A1A24] transition-colors resize-y leading-relaxed font-bold"
                    dir="rtl"
                  />
                </div>
              </div>
            ...
            {isTeacher && (
              <div className="bg-white dark:bg-[#111827] rounded-3xl shadow-sm border border-gray-200 dark:border-slate-800 flex flex-col p-5 space-y-4">
                <h3 className="font-black text-sm text-gray-900 dark:text-white flex items-center gap-2 border-b border-gray-100 dark:border-slate-800 pb-3">
                  <Sparkles className="w-4.5 h-4.5 text-sky-600 dark:text-cyan-400 animate-pulse" />
                  <span>لوحة تحكم المعلم السريعة</span>
                </h3>
                
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-gray-50 dark:bg-[#090D16]/40 p-3 rounded-2xl text-center border border-transparent hover:border-sky-500/20 transition-all">
                    <span className="block text-[9px] font-black text-gray-400 mb-0.5">إجمالي الطلاب</span>
                    <span className="text-base font-black text-sky-600 dark:text-cyan-400">
                ...
                <div className="space-y-2 pt-1">
                  ...
                  <div className="flex items-center justify-between text-xs font-bold text-gray-500">
                    ...
                    <span className="text-sky-600 dark:text-cyan-400 font-black">{course.price === 0 ? "مجاني" : `${course.price} ج.م`}</span>
                  </div>
                  <div className="flex items-center justify-between text-xs font-bold text-gray-500">
        ...
        {showAddLesson && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="bg-white dark:bg-[#111827] rounded-3xl w-full max-w-lg relative z-10 shadow-2xl overflow-hidden flex flex-col max-h-[85vh]"
            >
              <div className="p-4 sm:p-6 border-b border-gray-100 dark:border-slate-800 flex justify-between items-center shrink-0">
                ...
                <form id="add-lesson-form" onSubmit={handleAddLesson} className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      className="w-full bg-gray-50 dark:bg-[#090D16] border border-gray-200 dark:border-slate-800 rounded-xl px-4 py-3 text-gray-900 dark:text-white focus:outline-none focus:border-sky-500 dark:border-cyan-400 focus:bg-white dark:focus:bg-[#1A1A24] transition-colors"
                    />
                  </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex flex-col gap-3">
                        <div 
                          ...
                          className="w-full h-32 border-2 border-dashed border-gray-300 dark:border-slate-800 rounded-xl flex flex-col items-center justify-center cursor-pointer hover:bg-gray-50 dark:hover:bg-[#1A1A24] transition-colors relative overflow-hidden"
                        >
                          {videoFile ? (
                            <div className="flex flex-col items-center text-sky-600 dark:text-cyan-400">
                        ...
                        <input
                          ...
                          className="w-full bg-gray-50 dark:bg-[#090D16] border border-gray-200 dark:border-slate-800 rounded-xl px-4 py-3 text-gray-900 dark:text-white focus:outline-none focus:border-sky-500 dark:border-cyan-400 focus:bg-white dark:focus:bg-[#1A1A24] transition-colors text-left"
                          dir="ltr"
                        />
                  ...
                  {isSubmitting && videoFile && (
                    <div className="space-y-1">
                      ...
                      <div className="w-full bg-gray-200 dark:bg-[#2D2D3D] rounded-full h-2">
                        <div className="bg-sky-600 dark:bg-cyan-500 h-2 rounded-full transition-all duration-300" style={{ width: `${Math.max(uploadProgress, 2)}%` }}></div>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea
                      ...
                      rows={3}
                      className="w-full bg-gray-50 dark:bg-[#090D16] border border-gray-200 dark:border-slate-800 rounded-xl px-4 py-3 text-gray-900 dark:text-white focus:outline-none focus:border-sky-500 dark:border-cyan-400 focus:bg-white dark:focus:bg-[#1A1A24] transition-colors resize-none"
                    />
                  </div>
                </form>
              </div>

              <div className="p-4 sm:p-6 border-t border-gray-100 dark:border-slate-800 flex justify-end gap-3 sm:gap-4 bg-gray-50 dark:bg-[#090D16] shrink-0">
                <button
                  type="button"
                ...
                <button
                  ...
                  disabled={isSubmitting}
                  className="bg-sky-600 dark:bg-cyan-500 text-white px-8 py-3 rounded-xl font-bold shadow-lg shadow-sky-500/20 dark:shadow-cyan-500/20 hover:bg-[#0077B6] dark:bg-[#B8860B] transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 border-0"
                >
                  {isSubmitting ? (
                    <>
        ...
        {lessonToDelete && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 20 }}
              className="bg-white dark:bg-[#111827] rounded-3xl w-full max-w-md relative z-10 shadow-2xl p-6 border border-gray-100 dark:border-slate-800 text-center"
            >
        ...
        {showCompletionRating && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="bg-white dark:bg-[#111827] w-full max-w-lg rounded-3xl overflow-hidden shadow-2xl border border-gray-200 dark:border-slate-800 z-10 relative flex flex-col max-h-[90vh]"
              dir="rtl"
            >
              {/* Top Banner Accent */}
                ...
                <div className="space-y-5">
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea
                      ...
                      className="w-full bg-gray-50 dark:bg-[#090D16] border border-gray-200 dark:border-slate-800 rounded-2xl p-4 text-xs text-gray-900 dark:text-white focus:outline-none focus:border-yellow-500 dark:focus:border-yellow-500 focus:bg-white dark:focus:bg-[#1A1A24] transition-colors resize-none leading-relaxed"
                      rows={3}
                    />
                  </div>
              ...
              {/* Footer */}
              <div className="bg-gray-50 dark:bg-[#15151F] px-6 py-4 border-t border-gray-150 dark:border-slate-800 flex items-center justify-end gap-3 shrink-0">
                <button
                  type="button"
                  onClick={() => setShowCompletionRating(false)}
                  className="px-5 py-2.5 rounded-xl border border-gray-200 dark:border-slate-800 hover:bg-gray-100 dark:hover:bg-[#222230] text-gray-700 dark:text-gray-300 text-xs font-black transition-colors cursor-pointer bg-transparent"
                >
                  إلغاء
                </button>
                <button
        ...
        {showPaymentModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm overflow-y-auto">
            <motion.div 
              ...
              className="bg-white dark:bg-[#111827] rounded-3xl w-full max-w-lg shadow-2xl border border-gray-150 dark:border-slate-800 flex flex-col overflow-hidden max-h-[90vh]"
              dir="rtl"
            >
              {/* Header */}
              <div className="px-6 py-5 border-b border-gray-100 dark:border-slate-800 bg-gray-50/50 dark:bg-[#1C1C28]/50">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-2">
                    <div className="p-2 bg-sky-500/10 dark:bg-cyan-400/10 rounded-xl text-sky-600 dark:text-cyan-400">
                ...
                <div className="flex p-1 bg-gray-100 dark:bg-[#090D16] rounded-2xl border border-gray-200 dark:border-slate-800">
                  <button
                  ...
                  {paymentMethod === 'wallet' ? (
                    ...
                    >
                      <div className="bg-blue-50/50 dark:bg-blue-950/10 border border-blue-100 dark:border-blue-900/30 rounded-3xl p-6 text-center space-y-4">
                        <div className="w-16 h-16 bg-white dark:bg-[#090D16] rounded-2xl flex items-center justify-center mx-auto shadow-sm border border-blue-100 dark:border-blue-900/30">
                          <Wallet className="w-8 h-8 text-sky-600 dark:text-cyan-400" />
                        </div>
                        <div>
                      ...
                      <div className="space-y-4">
                        <div className="flex items-center justify-between p-4 rounded-2xl bg-gray-50 dark:bg-[#090D16] border border-gray-100 dark:border-slate-800">
                      ...
                      <div className="pt-4 flex flex-col gap-3">
                        <button
                          ...
                          className="w-full bg-sky-600 dark:bg-cyan-500 hover:bg-sky-700 dark:hover:bg-cyan-600 text-white dark:text-[#0D0D12] py-4 rounded-2xl font-black text-sm shadow-lg shadow-sky-500/20 dark:shadow-cyan-500/10 disabled:opacity-50 disabled:grayscale transition-all flex items-center justify-center gap-2 cursor-pointer border-0"
                        >
                          {isPayingWithWallet ? (
                            <>
                              <Loader2 className="w-4 h-4 animate-spin" />
                    ...
                    >
                      ...
                      <form onSubmit={handlePaymentSubmit} className="p-6 space-y-5">
                        ...
                        <div className="bg-gray-50/50 dark:bg-[#090D16]/50 border border-gray-100 dark:border-slate-800 rounded-2xl p-4 space-y-4">
                          ...
                          <div className="space-y-3">
                            {isVodafoneCashEnabled && vodafoneCashNumber && (
                              <div className="flex flex-col gap-2 bg-white dark:bg-[#0D121F] border border-rose-100 dark:border-rose-950/30 rounded-xl p-3">
                                ...
                                <div className="flex items-center justify-between">
                                  ...
                                  <button
                                    ...
                                    className="p-2 bg-gray-50 hover:bg-rose-50 dark:bg-[#111827] dark:hover:bg-rose-950/30 text-rose-600 dark:text-rose-400 rounded-lg transition-all active:scale-95 border border-gray-100 dark:border-slate-800"
                                  >
                            ...
                            {isInstapayEnabled && instapayHandle && (
                              <div className="flex flex-col gap-2 bg-white dark:bg-[#0D121F] border border-purple-100 dark:border-purple-900/30 rounded-xl p-3">
                                ...
                                <div className="flex items-center justify-between">
                                  ...
                                  <button
                                    ...
                                    className="p-2 bg-gray-50 hover:bg-purple-50 dark:bg-[#111827] dark:hover:bg-purple-950/30 text-purple-600 dark:text-purple-400 rounded-lg transition-all active:scale-95 border border-gray-100 dark:border-slate-800"
                                  >
                            ...
                            {isBankAccountEnabled && bankAccountDetails && (
                              <div className="flex flex-col gap-2 bg-white dark:bg-[#0D121F] border border-blue-100 dark:border-blue-900/30 rounded-xl p-3">
                                ...
                                <div className="flex items-start justify-between">
                                  ...
                                  <button
                                    ...
                                    className="p-2 bg-gray-50 hover:bg-blue-50 dark:bg-[#111827] dark:hover:bg-blue-950/30 text-blue-600 dark:text-blue-400 rounded-lg transition-all active:scale-95 border border-gray-100 dark:border-slate-800 shrink-0 mr-3"
                                  >
                                    <Copy className="w-4 h-4" />
                                  </button>
                                </div>
                        ...
                        <div className="space-y-1.5">
                          ...
                          <input
                            ...
                            className="w-full bg-gray-50 dark:bg-[#090D16] border border-gray-200 dark:border-slate-800 rounded-xl px-4 py-3 outline-none focus:border-sky-500 dark:focus:border-cyan-500 dark:text-white font-bold text-xs font-sans"
                          />
                        </div>
                        ...
                        <div className="space-y-1.5">
                          ...
                          <input
                            ...
                            className="w-full bg-gray-50 dark:bg-[#090D16] border border-gray-200 dark:border-slate-800 rounded-xl px-4 py-3 outline-none focus:border-sky-500 dark:focus:border-cyan-500 dark:text-white font-bold text-xs font-sans text-right"
                            dir="ltr"
                          />
                        </div>
                        ...
                        <div className="space-y-1.5">
                          ...
                          <div 
                            ...
                            className="w-full h-40 border-2 border-dashed border-gray-300 dark:border-slate-800 rounded-2xl flex flex-col items-center justify-center cursor-pointer hover:bg-gray-50 dark:hover:bg-[#13131C] transition-all relative overflow-hidden p-4 group"
                          >
                            {paymentScreenshotPreview ? (
                              ...
                              <div className="flex flex-col items-center text-gray-500 dark:text-gray-400">
                                <div className="p-3 bg-gray-100 dark:bg-[#111827] rounded-2xl mb-2 text-rose-500">
