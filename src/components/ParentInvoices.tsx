      ...
      <div className="bg-white dark:bg-[#111827] p-6 rounded-3xl border border-gray-200 dark:border-slate-800 shadow-sm">
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 font-bold mt-1">
              سجل مشتريات الطالب: <span className="text-sky-600">{linkedStudent?.name || 'غير متصل'}</span>
            </p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {invoices.map((inv) => (
              <div key={inv.id} className="bg-gray-50 dark:bg-[#0D121F] p-5 rounded-2xl border border-gray-150 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-all hover:border-sky-500/30">
                <div className="flex items-start gap-4">
                  <div className="w-10 h-10 bg-white dark:bg-[#111827] rounded-xl flex items-center justify-center border border-gray-100 dark:border-slate-800 shrink-0">
